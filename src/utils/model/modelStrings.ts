...
function getOpenAIModelStrings(): ModelStrings {
  ...
  const smallFastModel = process.env.OPENAI_SMALL_FAST_MODEL || defaultModel
  return {
...
function getGeminiModelStrings(): ModelStrings {
  const defaultModel = process.env.GEMINI_MODEL || 'gemini-2.5-flash'
  const smallFastModel = process.env.GEMINI_SMALL_FAST_MODEL || defaultModel
  return {
    haiku35: smallFastModel,
    haiku45: smallFastModel,
    sonnet35: defaultModel,
...
function getBuiltinModelStrings(provider: APIProvider): ModelStrings {
  ...
  if (provider === 'gemini') {
    return getGeminiModelStrings()
  }
  const out = {} as ModelStrings
  for (const key of MODEL_KEYS) {
    out[key] = ALL_MODEL_CONFIGS[key][provider as BuiltinAPIProvider]
  }
  return out
}
...
export function refreshModelStringsForCurrentProvider(): void {
  if (getAPIProvider() === 'bedrock') {
    void updateBedrockModelStrings()
    return
  }
